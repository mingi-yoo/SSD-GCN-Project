...
uint64_t BufferInterface::ShedRow() // Give hint to MACController
{
	uint64_t ret;
	if (!isA)
...
uint64_t BufferInterface::ShedCol()
{
	uint64_t ret;
	if (!isA)
		ret = data->ifcolindex[shed_col];
	else
		ret = data->adjcolindex[shed_col];
